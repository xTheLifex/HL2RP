...
Clockwork.chatBox:RegisterClass("cp_adminhelp", nil, function(info)
	...
	Clockwork.chatBox:Add(info.filtered, info.icon, PLUGIN.color, "[ADMIN] ", "@AHelp ", color, info.name, " (", color, info.speaker:SteamName(), "): ", info.text);
end);

-- Admin chat
Clockwork.chatBox:RegisterClass("cp_adminchat", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, info.icon, PLUGIN.color, "[ADMIN] ", "@", _team.GetColor(info.speaker:Team()), info.name, ": ", info.text);
end);

-- Updated admin chat, this one is to update the default Clockwork adminchat channel
Clockwork.chatBox:RegisterClass("priv", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, info.icon, PLUGIN.color, "[ADMIN] ", "@", _team.GetColor(info.speaker:Team()), info.name, ": ", info.text);
end);

Clockwork.chatBox:RegisterClass("cp_announce", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, "icon16/information.png", PLUGIN.color, "[SERVER ANNOUNCEMENT] ", info.text);
end);

-- Warning
Clockwork.chatBox:RegisterClass("cp_warn_player", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, info.icon, PLUGIN.color, "[ADMIN] ", "@WARNING ", "from ", PLUGIN.color, info.speaker:SteamName(), ": ", info.text);
end);
Clockwork.chatBox:RegisterClass("cp_warn_admin", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, info.icon, PLUGIN.color, "[ADMIN] ", _team.GetColor(info.speaker:Team()), info.speaker:SteamName(), " has warned ",
...
Clockwork.chatBox:RegisterClass("cp_deathnotification", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, "icon16/exclamation.png", PLUGIN.color, "[ADMIN] ", info.data.attackerColor, info.data.attackerName, " has killed ", info.data.playerColor, info.data.playerName, "!");
end);

-- Admin Approve message
Clockwork.chatBox:RegisterClass("cp_unapprovedplayer", nil, function(info)
	Clockwork.chatBox:Add(info.filtered, "icon16/new.png", PLUGIN.color, "[ADMIN] ", info.data.color, info.text, " (OOC: ", info.data.ooc, ") has spawned and isn't approved yet.");
end);
